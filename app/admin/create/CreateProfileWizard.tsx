...
type ApiSuccess = {
  profile?: { id?: string; qrId?: string; packageId?: string };
  setup?: { url?: string };
  setupUrl?: string;
  profileId?: string;
  publicUrl?: string;
...
function extractSuccess(data: ApiSuccess): {
  ...
  const packageId = data.profile?.packageId;
  if (!setupUrl || !profileId || !publicUrl || !packageId) return null;
  return { setupUrl, profileId, publicUrl, packageId };
}

const STEPS = ["Information", "Package", "Review", "QR & link"];

export function CreateProfileWizard({ packages }: { packages: PackageInfo[] }) {
  const nameId = useId();
  const errorId = useId();
  ...
  if (state.status === "success") {
    const pkg = packages.find(p => p.id === state.packageId) ?? packages[0];
    return (
          ...
          <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-foreground-secondary">
                {pkg.name} · {pkg.retentionYears} years. Share the
        ...
        {wizardStep === 1 ? (
          <div>
            ...
            <p className="mt-3 text-xs text-foreground-muted">
              {limits.maxImages} photos ·{" "}
              {limits.maxVideoSeconds}s video ·{" "}
              {limits.maxAudioSeconds}s audio ·{" "}
